...
const Home: NextPage<any> = ({res , page}) => {
  ...
  return (
    ...
    <Header />
      ...
      {/* pagination */}
      <Pagination pages={res.metadata.page_count} incrementPage={incrementPage} decrementPage={decrementPage} currentPage={res.metadata.current_page} />
      {/* footer */}
    </div>
  )
}
...
export const getServerSideProps:GetServerSideProps = async ({query:{page = 1}})=>{
  const response = await fetch(`https://moviesapi.ir/api/v1/movies?page=${page}`)
